...
# Initialize orchestrator for streaming first message
...
async def start_new_game(callback: CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        # Add game rules quant ONLY for non-sandbox worlds
        # Sandbox has its own creation-focused quant and doesn't need generic rules
        if world_id != "sandbox":
            from core.models import Quant, QuantType
            rules_quant = Quant(
                id="правила_игры",
        ...
        # Process first turn with streaming
        try:
            logger.info(f"Processing first turn with streaming for session {new_session.id}")
            ...
            async def on_narrative_update(narrative: str):
                ...
                # Format with header for first message
            ...
            if settings.enable_streaming:
                logger.info("🎬 Using streaming for first message")
                ...
                logger.info("📄 Using non-streaming mode for first message")
                result = await orchestrator.process_turn(
                    session_id=new_session.id,
                    user_message="Объясни правила игры, и давай создадим персонажа"
                )
            ...
            header = loc.get_story_started_header()
            final_message = f"{header}{reply_text}"
            ...
            if len(final_html) > 4000:
                ...
                logger.info(f"First message split into {len(chunks)} chunks")
                
                if settings.enable_streaming and updater:
            ...
            logger.error(f"Error processing first turn: {e}", exc_info=True)
            await loading.replace_with_text(
                f"❌ {loc.get_error_message()}\n\n{str(e)[:100]}"
            )
